...
func runPayloads(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: nox payloads generate|validate|list <session-id>")
	}
	sessionID := args[1]
...
func runAD(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: nox ad enum|kerberoast|bloodhound <session-id>")
	}
	...
	default:
		return fmt.Errorf("usage: nox ad enum|kerberoast <session-id> or nox ad bloodhound export <session-id>")
	}
}
...
func runBurp(args []string) error {
	if len(args) < 2 {
		return fmt.Errorf("usage: nox burp status|push-scope|pull-issues <session-id> or nox burp export scope|findings <session-id> --output file.xml")
	}
