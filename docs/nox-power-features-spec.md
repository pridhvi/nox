# NOX — Power Features Specification
...
When a vulnerability is confirmed by a dynamic tool, NOX's LLM generates context-aware custom payloads tuned to the specific target environment. This bridges the gap between "vulnerability detected" and "exploitation ready" — producing payloads that account for the detected WAF, database engine, CSP policy, and application behaviour rather than relying on generic tool defaults.
...
Before generating payloads, NOX must identify whether a WAF is present and which one. Extend the fingerprint phase to include WAF detection using the following signals:
...
```
nox payloads generate <session-id> --finding <finding-id>
nox payloads generate <session-id> --all           # generate for all confirmed findings
nox payloads validate <session-id> --finding <finding-id>
nox payloads list <session-id> [--validated-only]
```
...
Use `github.com/robfig/cron/v3` for cron scheduling. The scheduler starts when `nox serve` runs and loads all enabled `monitor_configs` from the DB.
...
```json
{
  "blocks": [
    {
      "type": "header",
      "text": { "type": "plain_text", "text": "🔴 NOX Alert — New Critical Finding" }
    },
    {
      "type": "section",
      "fields": [
...
```
nox monitor create --target example.com --schedule "0 */6 * * *" --name "Example Corp"
nox monitor list
nox monitor enable <config-id>
nox monitor disable <config-id>
nox monitor run <config-id>           # trigger an immediate run
nox monitor changes <config-id>       # show recent changes
...
credential_testing:
  ...
  default_creds: true
  spray: true
  spray_wordlist: ~/.nox/wordlists/spray.txt  # custom wordlist path
...
Makes NOX scans survivable against real defences. Adds adaptive rate limiting, traffic blending, proxy chain support, and WAF-aware scanning profiles. Activated with `--stealth` flag or configurable per-session.
...
|---|---|---|---|---|
| aggressive | 100 | none | nox/1.0 | Existing default behaviour |
...
```
nox scan --target example.com --stealth
nox scan --target example.com --profile paranoid
nox scan --target example.com --proxy socks5://127.0.0.1:9050
nox scan --target example.com --proxy-list proxies.txt
nox scan --target example.com --rate-limit 5
nox scan --target example.com --jitter 2s
```
...
Two-way integration between NOX and Burp Suite. Import discovered endpoints and issues from Burp, export NOX-discovered attack surface to Burp's scope, and use Burp Collaborator or Interactsh as the out-of-band callback server for SSRF and blind injection tests.

### Integration modes

**Mode A: File-based (no Burp running required)**
- Import Burp XML project exports
- Export NOX scope as Burp target scope XML
- Import Burp scanner issues as NOX findings
...
- Pull live scanner results into NOX
...
├── exporter.go        # export NOX data to Burp format
...
func (b *BurpXMLImporter) Import(ctx context.Context, xmlPath string, sessionID string) (ImportResult, error) {
    ...
    //
    // Deduplicate: don't import findings that match existing NOX findings
...
func (c *BurpAPIClient) GetScanResults(ctx context.Context, scanID string) ([]BurpIssue, error) {
    ...
    // Parse issues, convert to NOX Finding records
}
...
Export NOX session targets as Burp scope XML for import into Burp:
...
nox burp push-scope <session-id>           # requires Burp running
nox burp pull-issues <session-id>          # requires Burp running
nox burp collaborator set --provider interactsh --url https://interactsh.com
```
...
> "This document specifies 8 enhancement modules for NOX. Which would you like to implement?
>
...
*End of NOX Power Features Specification — Version 1.0*
