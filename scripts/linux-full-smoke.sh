...
fail() {
  echo "Linux full smoke failed: $*" >&2
...
assert_count_at_least "$dynamic_db" "SELECT COUNT(*) FROM findings WHERE tool_id IN ('security-headers','graphql-introspection','openapi-discovery','js-secret-scan','cors-check');" 5 "built-in dynamic findings"
...
assert_count_at_least "$lean_db" "SELECT COUNT(*) FROM tool_runs;" 8 "lean tool runs"
...
assert_count_at_least "$audit_db" "SELECT COUNT(*) FROM source_findings;" 6 "source findings"
