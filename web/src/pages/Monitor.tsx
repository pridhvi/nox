      ...
      <div className="operator-grid">
        <section className="scan-form monitor-form">
          ...
          <form className="form-grid" onSubmit={(event) => { event.preventDefault(); createMutation.mutate(); }}>
            ...
            <label>Phases
              <input value={form.enabled_phases.join(",")} onChange={(event) => setForm({ ...form, enabled_phases: csv(event.target.value) })} />
            </label>
            <label>Alert On
              <input value={form.alert_on.join(",")} onChange={(event) => setForm({ ...form, alert_on: csv(event.target.value) })} placeholder="new_finding, new_host" />
            </label>
